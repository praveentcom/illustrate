...
class G_REPLICATE_FLUX_DEV_EDIT: ImageGenerationProtocol {
    func getCreditsUsed(request: ImageGenerationRequest) -> Double {
        return 0.03;
    }

    let model: PartnerModel = partnerModels.first(where: { $0.modelCode == EnumPartnerModelCode.REPLICATE_FLUX_DEV_EDIT })!
    ...
    struct ServiceRequest: Codable {
        ...
        init(prompt: String, aspectRatio: String, image: String) {
            self.prompt = prompt
            self.image = image
            self.num_outputs = 1

...
class G_REPLICATE_SEEDREAM_4_5: ImageGenerationProtocol {
    func getCreditsUsed(request: ImageGenerationRequest) -> Double {
        return CostEstimator.getCreditsUsed(request: request)
    }

    let model: ConnectionModel = ConnectionService.shared.model(by: EnumConnectionModelCode.REPLICATE_SEEDREAM_4_5.modelId.uuidString)!
    ...
    struct ServiceRequest: Codable {
        ...
        init(prompt: String, aspectRatio: String = "1:1", size: String = "2K") {
            self.prompt = prompt

...
class G_REPLICATE_SEEDREAM_4_5_EDIT: ImageGenerationProtocol {
    func getCreditsUsed(request: ImageGenerationRequest) -> Double {
        return CostEstimator.getCreditsUsed(request: request)
    }

    let model: ConnectionModel = ConnectionService.shared.model(by: EnumConnectionModelCode.REPLICATE_SEEDREAM_4_5_EDIT.modelId.uuidString)!
    ...
    struct ServiceRequest: Codable {
        ...
        init(prompt: String, imageInput: [String], aspectRatio: String = "match_input_image", size: String = "2K") {
            self.prompt = prompt
    ...
    func createServiceRequest(request: ImageGenerationRequest, imageUrl: String) -> ServiceRequest {
        return ServiceRequest(
            prompt: request.artVariant != EnumArtVariant.NORMAL ? "\(request.artVariant.rawValue) - \(request.prompt)" : request.prompt,
            imageInput: [imageUrl]
        )

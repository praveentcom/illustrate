...
class G_REPLICATE_SEEDREAM_4: ImageGenerationProtocol {
    func getCreditsUsed(request: ImageGenerationRequest) -> Double {
        return CostEstimator.getCreditsUsed(request: request)
    }

    let model: ConnectionModel = ConnectionService.shared.model(by: EnumConnectionModelCode.REPLICATE_SEEDREAM_4.modelId.uuidString)!
    ...
    struct ServiceRequest: Codable {
        ...
        init(prompt: String, aspectRatio: String, size: String = "2K", enhancePrompt: Bool = true) {
            self.prompt = prompt
            self.aspect_ratio = aspectRatio
            self.size = size
            self.enhance_prompt = enhancePrompt
            self.sequential_image_generation = "disabled"
        }
    }

    private func mapToSeedream4AspectRatio(dimension: String) -> String {
        let aspectRatio = getAspectRatio(dimension: dimension)
        let ratio = aspectRatio.ratio
        
        let supportedRatios = ["1:1", "4:3", "3:4", "16:9", "9:16", "3:2", "2:3", "21:9"]
    ...
    func transformRequest(request: ImageGenerationRequest) -> ServiceRequest {
        let aspectRatio = mapToSeedream4AspectRatio(dimension: request.artDimensions)

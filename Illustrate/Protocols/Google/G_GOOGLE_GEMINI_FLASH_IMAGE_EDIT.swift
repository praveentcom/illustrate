...
class G_GOOGLE_GEMINI_FLASH_IMAGE_EDIT: ImageGenerationProtocol {
    func getCreditsUsed(request: ImageGenerationRequest) -> Double {
        return CostEstimator.getCreditsUsed(request: request)
    }

    let model: ConnectionModel = ConnectionService.shared.model(by: EnumConnectionModelCode.GOOGLE_GEMINI_FLASH_IMAGE_EDIT.modelId.uuidString)!
    ...
    struct ServiceRequest: Codable {
        ...
        init(prompt: String, imageBase64: String, mimeType: String = "image/png", aspectRatio: String?) {
            self.contents = [
                Content(parts: [
    ...
    func transformRequest(request: ImageGenerationRequest) -> ServiceRequest {
        ...
        return ServiceRequest(prompt: request.prompt, imageBase64: imageBase64, aspectRatio: aspectRatio)
    }

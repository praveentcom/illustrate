...
struct SearchReplaceImageView: View {
    ...
    func getSupportedModels() -> [ProviderModel] {
        ...
        return ProviderService.shared.allModels.filter {
    ...
    var body: some View {
        VStack {
            if selectedModelId != "" && !providerKeys.isEmpty {
                        ...
                        Picker("Provider", selection: $selectedProviderId) {
                            ForEach(
                                providers.filter { provider in
                                    ...
                                    ProviderService.shared.allModels.contains { $0.providerId == provider.providerId && $0.modelSetType == .EDIT_REPLACE && $0.active }
                                }, id: \.providerId
                            ) { provider in
                                HStack {
#if !os(macOS)
                ...
                PendingProviderView(setType: .EDIT_REPLACE)
            }
        }
        .onAppear {

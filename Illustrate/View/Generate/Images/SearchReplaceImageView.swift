...
struct SearchReplaceImageView: View {
    ...
    func getSupportedModels() -> [PartnerModel] {
        ...
        return partnerModels.filter({ $0.modelSetType == EnumSetType.EDIT_REPLACE && $0.partnerId.uuidString == selectedPartnerId })
    }
    ...
    private enum Field: Int, CaseIterable {
        case prompt, searchPrompt, negativePrompt
    }
    ...
    var body: some View {
                        ...
                        Picker("Partner", selection: $selectedPartnerId) {
                            ForEach(
                                partners.filter { partner in
                                    ...
                                    partnerModels.contains { $0.partnerId == partner.partnerId && $0.modelSetType == .EDIT_REPLACE }
                                }, id: \.partnerId
                            ) { partner in
                                HStack {
                                    #if !os(macOS)
        ...
        .navigationTitle("Search and Replace Image")
    }
}

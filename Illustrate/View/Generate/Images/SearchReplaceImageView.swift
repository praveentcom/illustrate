...
struct SearchReplaceImageView: View {
    ...
    func getSupportedModels() -> [ConnectionModel] {
        ...
        return connectionModels.filter({ $0.modelSetType == EnumSetType.EDIT_REPLACE && $0.connectionId.uuidString == selectedConnectionId })
    }
                        ...
                        Picker("Connection", selection: $selectedConnectionId) {
                            ForEach(
                                connections.filter { connection in
                                    ...
                                    connectionModels.contains { $0.connectionId == connection.connectionId && $0.modelSetType == .EDIT_REPLACE }
                                }, id: \.connectionId
                            ) { connection in
                                HStack {
                                    #if !os(macOS)

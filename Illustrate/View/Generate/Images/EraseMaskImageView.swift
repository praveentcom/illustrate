...
struct EraseMaskImageView: View {
    ...
    func getSupportedModels() -> [ProviderModel] {
        ...
        return ProviderService.shared.models(for: EnumSetType.EDIT_MASK_ERASE).filter {
            ...
            if selectedModelId != "" && !providerKeys.isEmpty {
                Form {
                            ...
                            ForEach(
                                providers.filter { provider in
                                    providerKeys.contains { $0.providerId == provider.providerId } &&
                                        ProviderService.shared.allModels.contains { $0.providerId == provider.providerId && $0.modelSetType == .EDIT_MASK_ERASE && $0.active }
                                }, id: \.providerId
                            ) { provider in
                                HStack {
                                    #if !os(macOS)
                    ...
                    if getSelectedModel()?.modelSupportedParams.prompt ?? false {
                        Section(header: Text("Erase instructions")) {
                            TextField("What specifics do you want to erase?", text: $prompt, prompt: Text("Eg. Face in the image"), axis: .vertical)
                ...
                PendingProviderView(setType: .EDIT_MASK_ERASE)
            }
        }
        .onAppear {

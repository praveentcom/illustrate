...
struct EditPromptImageView: View {
    ...
    func getSupportedModels() -> [ProviderModel] {
        ...
        return ProviderService.shared.models(for: EnumSetType.EDIT_PROMPT).filter {
    ...
    var body: some View {
        VStack {
            if selectedModelId != "" && !providerKeys.isEmpty {
                Form {
                            ...
                            ForEach(
                                providers.filter { provider in
                                    providerKeys.contains { $0.providerId == provider.providerId } &&
                                        ProviderService.shared.allModels.contains { $0.providerId == provider.providerId && $0.modelSetType == .EDIT_PROMPT }
                                }, id: \.providerId
                            ) { provider in
                                HStack {
                                    #if !os(macOS)
                    ...
                    if getSelectedModel()?.modelSupportedParams.prompt ?? false {
                        Section(header: Text("Edit instructions")) {
                            TextField("What's your image about (or) edit instruction?", text: $prompt, prompt: Text("Eg. Add text with word 'cat' to the image"), axis: .vertical)
                    ...
                    Section {
                        EstimatedCostView(
                            cost: CostEstimator.estimatedImageCost(
                                modelCode: getSelectedModel()?.modelCode ?? .GOOGLE_GEMINI_FLASH_IMAGE_EDIT,
                                quality: artQuality,
                ...
                PendingProviderView(setType: .EDIT_PROMPT)
            }
        }
        .onAppear {

...
struct RemoveBackgroundImageView: View {
    ...
    func getSupportedModels() -> [ConnectionModel] {
        ...
        return connectionModels.filter({ $0.modelSetType == EnumSetType.REMOVE_BACKGROUND && $0.connectionId.uuidString == selectedConnectionId })
    }
                        ...
                        Picker("Connection", selection: $selectedConnectionId) {
                            ForEach(
                                connections.filter { connection in
                                    ...
                                    connectionModels.contains { $0.connectionId == connection.connectionId && $0.modelSetType == .REMOVE_BACKGROUND }
                                }, id: \.connectionId
                            ) { connection in
                                HStack {
                                    #if !os(macOS)

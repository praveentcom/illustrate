...
struct RemoveBackgroundImageView: View {
    ...
    func getSupportedModels() -> [ProviderModel] {
        ...
        return ProviderService.shared.models(for: EnumSetType.REMOVE_BACKGROUND).filter {
            ...
            if selectedModelId != "" && !providerKeys.isEmpty {
                Form {
                        ...
                        Picker("Provider", selection: $selectedProviderId) {
                            ForEach(
                                providers.filter { provider in
                                    ...
                                    ProviderService.shared.allModels.contains { $0.providerId == provider.providerId && $0.modelSetType == .REMOVE_BACKGROUND && $0.active }
                                }, id: \.providerId
                            ) { provider in
                                HStack {
#if !os(macOS)
                    ...
                    if getSelectedModel()?.modelSupportedParams.prompt ?? false {
                        Section(header: Text("Remove instructions")) {
                            TextField("What specifics do you want to remove?", text: $prompt, prompt: Text("Eg. Face in the image"), axis: .vertical)
                    ...
                    Section {
                        EstimatedCostView(
                            cost: CostEstimator.estimatedImageCost(
                                modelCode: getSelectedModel()?.modelCode ?? .STABILITY_REMOVE_BACKGROUND,
                                quality: artQuality,
                ...
                PendingProviderView(setType: .REMOVE_BACKGROUND)
            }
        }
        .onAppear {

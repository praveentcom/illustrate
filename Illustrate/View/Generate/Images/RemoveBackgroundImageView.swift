...
struct RemoveBackgroundImageView: View {
    ...
    func getSupportedModels() -> [PartnerModel] {
        ...
        return partnerModels.filter({ $0.modelSetType == EnumSetType.REMOVE_BACKGROUND && $0.partnerId.uuidString == selectedPartnerId })
    }
                        ...
                        Picker("Partner", selection: $selectedPartnerId) {
                            ForEach(
                                partners.filter { partner in
                                    ...
                                    partnerModels.contains { $0.partnerId == partner.partnerId && $0.modelSetType == .REMOVE_BACKGROUND }
                                }, id: \.partnerId
                            ) { partner in
                                HStack {
                                    #if !os(macOS)

...
struct EditExpandImageView: View {
    ...
    func getSupportedModels() -> [ProviderModel] {
        ...
        return ProviderService.shared.models(for: EnumSetType.EDIT_EXPAND).filter {
    ...
    var body: some View {
        VStack {
            if selectedModelId != "" && !providerKeys.isEmpty {
                        ...
                        Picker("Provider", selection: $selectedProviderId) {
                            ForEach(
                                providers.filter { provider in
                                    ...
                                    ProviderService.shared.allModels.contains { $0.providerId == provider.providerId && $0.modelSetType == .EDIT_EXPAND }
                                }, id: \.providerId
                            ) { provider in
                                HStack {
#if !os(macOS)
                ...
                PendingProviderView(setType: .EDIT_EXPAND)
            }
        }
        .onAppear {
